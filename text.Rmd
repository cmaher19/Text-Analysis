...
peterpan %>%
  count(word) %>%
  with(wordcloud(word, n, max.words = 50, scale=c(2, 0.5)))
...
peterpan %>%
  ...
  comparison.cloud(colors = c("orchid", "seagreen2"), max.words = 75, scale = c(2,0.5))
...
peterpan %>%
  ...
  count(word, sentiment, sort = TRUE) %>%
  acast(word ~ sentiment, value.var = "n", fill=1) %>%
  comparison.cloud(colors = c("orchid", "seagreen2"), max.words = 75, scale = c(1,0.5))
```
...
network_graph <- ggraph(bigram_graph, layout = "fr") + geom_edge_link(aes(edge_alpha = n), show.legend = FALSE, arrow = a, end_cap = circle(.07, "inches")) + geom_node_point(color = "plum", size = 3) + geom_node_text(aes(label = name), vjust = 1, hjust = 1) + theme_void()


g1 <- clean_bigrams %>% filter(!is.na(word1), !is.na(word2), n>3) %>% graph_from_data_frame(directed = T)
plot(g1)
...
Word coocurrences
```{r}
# Break text down into sections of 5 lines
...
library(widyr)
# Look at most common word pairs - THIS PRODUCES ERROR, look at it pls
...
Correlations
```{r}
# Looking at actual correlations - THIS PRODUCES ERROR
